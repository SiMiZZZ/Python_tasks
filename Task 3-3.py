...
class MultipleReader:
    ...
    def reader(self):
        with multiprocessing.Pool(len(self.file_names)) as pool:
...
class MultipleStatistics:
    ...
    def get_statistic(self, vacancies):
        ...
        with multiprocessing.Pool(len(vacancies)) as pool:
            statistics = pool.starmap(Statistics, tuple(map(lambda x: (x, self.vacancie_name), vacancies)))
        return statistics
